...
from .models import Product, Contact, Orders,OrderUpdate,signup,  signup, RentInfo,ThemeProduct
...
from django.shortcuts import render, redirect
from django.contrib import messages
from .models import signup  # Adjust the import as per your actual model name
...
def search(request):
    ...
    # theme_query= request.GET.get('search')
    # theme_allProds = []
    # theme_catprods = ThemeProduct.objects.values('theme_category', 'theme_product_id')
    # theme_cats = {item['theme_category'] for item in catprods}  
    # for thenme_cat in theme_cats:
    #     theme_prodtemp = ThemeProduct.objects.filter(theme_category=theme_cat)
    #     theme_prod=[item for item in prodtemp if searchMatch(theme_query, item)]
    #     n = len(prod)
    #     nSlides = n // 4 + ceil((n / 4) - (n // 4))
    #     if len(prod)!= 0:
    #         allProds.append([prod, range(1, nSlides), nSlides])
